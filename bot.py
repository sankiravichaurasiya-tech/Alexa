...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = "8487451759:AAG6V7MHZsMjNidwN_q7sEbZm3MhgJfhY4w"   # ← ADD BOT TOKEN
TOKEN = (
...
try:
    OWNER_ID = int(os.environ.get("OWNER_ID", "7766306643"))
...
BRAND       = "ALEXA нoѕтιηg ＲΒOT"
BRAND_VER   = "v2.1"
BRAND_TAG   = f"{BRAND} {BRAND_VER}"
SUPPORT_USR = "@xchumt"
UPDATE_CH   = "https://t.me/a3lxe"
...
_PHOTO_SPECS: Dict[str, Tuple[str, str, str]] = {
    ...
    "welcome":   ("Wᴇʟᴄᴏᴍᴇ",         "#0F172A", "ALEXA Hᴏꜱᴛɪɴɢ"),
...
class KeyRing:
    ...
    def _gh_request(self, method: str, path: str, **kw) -> Optional[requests.Response]:
        ...
        h.setdefault("User-Agent", "ALEXA-hosting-rbot/2.1")
        try:
...
def _gh_api(endpoint: str, token: Optional[str] = None,
    ...
    req.add_header("User-Agent",    "ALEXAHostingBot/2.0")
    if body:
...
def action_adm_wh_test(call: types.CallbackQuery) -> None:
    ...
    def _bg() -> None:
        try:
            import urllib.request as _ur
            import json as _j
            payload = _j.dumps({"test": True, "ts": ts_iso(), "from": "ALEXAHostingBot"}).encode()
...
def _do_export_data(admin_uid: int) -> Path:
    ...
    target = out / f"ALEXA_export_{stamp}.zip"
...
_HELP_PAGES = {
    "main": {
        "title": "📚 Help Centre",
        "text": (
            "Welcome to <b>ALEXAHosting Bot</b>\n\n"
            "<b>Quick Start</b>\n"
...
def action_adm_export_full_db(call):
    ...
    fname = f"ALEXA_db_{now_utc().strftime('%Y%m%d_%H%M%S')}.json"
    import io
...
def action_adm_export_users_csv(call):
    ...
    fname = f"ALEXA_users_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_bots_csv(call):
    ...
    fname = f"ALEXA_bots_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_trans_csv(call):
    ...
    fname = f"ALEXA_transactions_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_audit_csv(call):
    ...
    fname = f"ALEXA_audit_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
    ...
    try:
        ...
        with target.open("rb") as fh:
            bot.send_document(
                ...
                parse_mode="HTML",
                visible_file_name=f"ALEXA_backup_{stamp}.zip",
            )
...
def render_referral(call: types.CallbackQuery) -> None:
    ...
    try:
        ...
        link = f"https://t.me/ALEXARBOT?start={uid}"
    cap = (
        f"<b>{G['users']} {sc('Referral')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Your link', link)}\n"
